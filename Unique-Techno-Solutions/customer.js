...
function renderNotifications(){const box=document.getElementById('notificationList');if(!box)return;if(!notifications.length){box.innerHTML='<div class="notification-empty">No notifications.</div>';return}box.innerHTML=notifications.map(n=>`<div class="notification ${n.is_read?'':'unread'}" data-notification-id="${n.id}" onclick="markNotificationRead(${n.id})"><b>${esc(n.title)}</b><span>${esc(n.message)}</span><span>${esc(n.created_at)}</span></div>`).join('')}
async function markNotificationRead(id){if(!customer?.id||!id)return;try{await api('/api/customer/'+customer.id+'/notifications/read',{method:'POST',body:JSON.stringify({notification_id:id})});const n=notifications.find(x=>Number(x.id)===Number(id));if(n)n.is_read=1;renderNotifications();updateNotificationBadges();if(typeof renderDashboard==='function')renderDashboard();}catch(e){console.error(e)}}
