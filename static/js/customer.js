...
const api=async(u,o={})=>{const r=await fetch(u,{credentials:'same-origin',headers:{"Content-Type":"application/json",...(o.headers||{})},...o});const d=await r.json();if(!r.ok)throw Error(d.error||"Request failed");return d};
...
function sendOtp(){if(document.getElementById('phone').value.replace(/\D/g,'').length!==10)return alert('Enter a valid 10-digit mobile.');document.getElementById('phoneStep').style.display='none';document.getElementById('otpStep').classList.add('on')}
async function verify(){const otp=document.getElementById('otp').value.trim();if(otp!=='123456')return alert('Prototype OTP is 123456.');try{const d=await api('/api/customer/login',{method:'POST',body:JSON.stringify({phone:document.getElementById('phone').value,name:'Customer',otp})});customer=d.customer;document.body.classList.add('customer-auth');await Promise.all([loadServices(),loadOffers(),loadHistory(),loadProfile(),loadNotifications()]);renderDashboard();window.history.replaceState({},'', '/customer/');show('home');updateAccountControl()}catch(e){alert(e.message)}}
